    ...
    <div className="max-w-7xl mx-auto px-6 text-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
        {[
          ...
          <motion.div
            key={index}
            className="px-8 py-10 bg-gradient-to-br from-primary to-secondary shadow-2xl rounded-3xl text-white overflow-hidden relative transform transition-all duration-500 ease-in-out hover:scale-105"
      ...
      <div className="mt-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {[
            { title: 'AI Insights', description: 'Unlock smarter decision-making with AI-generated analytics.' },
            { title: 'Scalable', description: 'Flexible solutions for teams of any size, from solo projects to enterprises.' },
            { title: 'User-Friendly', description: 'Intuitive design ensures seamless navigation and productivity.' },
          ].map((item) => (
            <motion.div
              key={item.title}
              className="flex flex-col items-center justify-center px-8 py-10 bg-gradient-to-br from-primary to-secondary rounded-3xl text-white shadow-lg transform transition-all duration-500 ease-in-out hover:scale-105"
              initial={{ opacity: 0, y: 50 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 1, ease: 'easeOut' }}
            >
